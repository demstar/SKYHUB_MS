...
if (isset($_GET['error'])) {
    if ($_GET['error'] === 'invdate') {
        ...
        echo '<script>alert("Invalid contact info: Mobile number must be 10 digits")</script>';
...
<main>
    <div class="container mb-5">
        <div class="col-md-12 main-col">
            ...
            <form action="includes/pass_detail.inc.php" class="needs-validation mt-4" method="POST" id="passengerForm">
...
<script>
document.querySelector('#passengerForm').addEventListener('submit', function(e) {
    let isValid = true;
    let mobileFields = document.querySelectorAll('input[name="mobile[]"]');

    mobileFields.forEach(function(field) {
        if (field.value.length !== 10) {
            alert('Invalid contact info: Mobile number must be 10 digits.');
            isValid = false;
