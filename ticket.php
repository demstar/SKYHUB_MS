...
<style>
body {
  background: url('assets/images/ticket.jpg') no-repeat center center fixed; /* Adjust the path */
...
<main>
  <?php if(isset($_SESSION['userId'])) {   
    ...
    // Fetch passenger details using the current userId
    $sql_p = 'SELECT * FROM Passenger_profile WHERE user_id=?';
    $stmt_p = mysqli_stmt_init($conn);
    if(!mysqli_stmt_prepare($stmt_p, $sql_p)) {
        header('Location: ticket.php?error=sqlerror');
        exit();            
    } else {
        mysqli_stmt_bind_param($stmt_p, 'i', $_SESSION['userId']);          
        mysqli_stmt_execute($stmt_p);
        $result_p = mysqli_stmt_get_result($stmt_p);
        if($row_p = mysqli_fetch_assoc($result_p)) {
            // Continue with fetching ticket and flight details
            ...
            <h1 class="text-center mt-4 mb-4" style="color: white;">E-TICKETS</h1>
                ...
                if(!mysqli_stmt_prepare($stmt, $sql)) {
                    ...
                    while ($row = mysqli_fetch_assoc($result)) {   
                        // Use the fetched passenger details
                        ...
                        if(!mysqli_stmt_prepare($stmt_f, $sql_f)) {
                            ...
                            if($row_f = mysqli_fetch_assoc($result_f)) {
                                    ...
                                    <div class="row ">                                                     
                                        <div class="col">
                                            <h2 class="text-secondary mb-0 brand">
                                               SKYHUB AIRWAYS MANAGEMENT SYSTEM</h2> 
                                        </div>
                                        <div class="col">
                                ...
                                <div class="col-3 pl-0" style="background-color:#376b8d !important;
                                    ...
                                    <div class="row">  
                                        <div class="col">                                    
                                        <h3 class="text-light text-center">Booking Reference</h3>
                                        <h3 class="text-light text-center">'.$row['ticket_id'].'</h3> 
                                        </div>                                      
                                    </div> 
                                    <div class="row mt-4">  
                                        ...
                                        <form action="ticket.php" method="post">
                                            ...
                                            <button type="submit" name="cancel_but" class="btn btn-danger w-100">CANCEL TICKET</button>
                                        </form>
                                        </div>                                      
                                    </div>
                                </div>
            ...
            echo "<p class='text-center'>No passenger details found.</p>";
        }
    }  
  } else {
      echo "<p class='text-center'>You need to be logged in to view your tickets.</p>";
  }
  ?>
</main>
