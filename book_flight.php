...
// Move this logic to the very top before any output
if (isset($_POST['search_but'])) { 
    $dep_date = isset($_POST['dep_date']) ? $_POST['dep_date'] : '';                        
    $ret_date = isset($_POST['ret_date']) ? $_POST['ret_date'] : '';  
    $dep_city = isset($_POST['dep_city']) ? $_POST['dep_city'] : '';  
    $arr_city = isset($_POST['arr_city']) ? $_POST['arr_city'] : '';     
    $type = isset($_POST['type']) ? $_POST['type'] : '';
    $f_class = isset($_POST['f_class']) ? $_POST['f_class'] : '';
    $passengers = isset($_POST['passengers']) ? $_POST['passengers'] : 1;

    // Redirect early to avoid outputting anything before header modification
  ...
  <?php if (isset($_POST['search_but'])) { ?>
    <div class="container-md mt-2">
      <h1 class="display-4 text-center">
        FLIGHTS FROM: <br> <?php echo htmlspecialchars($dep_city); ?> to <?php echo htmlspecialchars($arr_city); ?>
      </h1>
      <table class="table table-striped table-bordered table-hover">
        ...
        <tbody>
          ...
          while ($row = mysqli_fetch_assoc($result)) {
            ...
            $status = '';
            $alert = 'alert-primary';  // Default alert class
            if ($row['status'] === '') {
            ...
            <tr class='text-center'>
              ...
              <td>
                <div>
                  <div class='alert $alert text-center mb-0 pt-1 pb-1' role='alert'>$status</div>
                </div>
              </td>
              <td>ksh $price</td>
            ";
...
<footer style="position: absolute; bottom: 0; width: 100%; height: 2.5rem;">
  <em>
    <h5 class="text-center p-0 brand mt-2">
      <img src="assets/images/plane-logo.png" height="40px" width="40px" alt="">
    </h5>
  </em>
  <p class="text-center">&copy; <?php echo date('Y');?> - SKYHUB AIRWAYS MANAGEMENT SYSTEM</p>
</footer>
